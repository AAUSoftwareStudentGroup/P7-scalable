module Session exposing (Details, Notification, PageType(..), Session(..), addNotification, createSessionFromLocalStorageValue, getNavKey, getNotifications, getNow, getUserToken, getUsername, login, logout, onChange, setNotifications, setNow)
...
addNotification session notificationBody =
    let
        notification =
            Notification notificationBody 3000 (getNow session)
    in
    case session of
