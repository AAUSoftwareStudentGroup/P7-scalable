...
module Recommender.DataLoad (Cell(..), loadSparseMatrix, loadQuestionData) where
...
loadSparseMatrix :: FilePath -> IO (Either String (Vector Cell))
loadSparseMatrix path = Csv.decode NoHeader <$> LBS.readFile path
